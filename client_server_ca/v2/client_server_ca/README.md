...
The server then repeats that entire process, except it doesn't yet because I was too lazy to implement it so it just trusts the client.\

Once the Server accepts the connection, it uses its AES key to encrypt the file being served, and encrypts the AES key using the clients public key \
