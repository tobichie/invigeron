...
def sign_and_recv_client_cert():
    ...
    signed_client_cert = ca.create_and_sign_client_cert(subject, public_key)
...
def extract_data(json_data):
    try:
        ...
        encrypted_key_hex = json_data["public_key"]
        nonce_hex = json_data["nonce_hex"]
        return original_name, output, encrypted_key_hex, nonce_hex
    except:
...
def main(port, output):
    ...
    if signature_verification(sock,  server_cert_crypto):
        ...
        original_name, output, server_public_hex, nonce_hex = extract_data(data)
        peer_public_key = bytes.fromhex(server_public_hex)
        print("Created json object")
        print("Original name:", original_name)
        print("Peer Key:", peer_public_key)
        # next, create own ECDHE key pair
        private_key = ec.generate_private_key(ec.SECP256R1())
        public_key = private_key.public_key()
        client_public_bytes = public_key.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint)
        peer_public_key = ec.EllipticCurvePublicKey.from_encoded_point(
            ec.SECP256R1(),
            bytes.fromhex(server_public_hex)
        )
        ...
        # Next the client needs to send its own public key so the server can derive a shared secret

        length_prefix = len(client_public_bytes).to_bytes(4, byteorder="big")
        sock.sendall(length_prefix + client_public_bytes)

        print("Made a shared secret")
        ciphertext = receive_exact(sock, int.from_bytes(sock.recv(4), "big"))
        aes_key = HKDF(
        ...
        print("Derived AES key from the shared secret")
